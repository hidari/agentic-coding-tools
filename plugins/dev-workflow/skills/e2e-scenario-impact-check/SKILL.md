...
- **構造的代替**: ローカル E2E は CI で動かないため人間の手元での実行に依存する。本 skill は静的検査でそのギャップを部分的に補う（ただし動的検証の代替ではない）

## いつ使うか

### 必ず使う

- dev-workflow:pre-merge-quality-gate Phase 1 の並列タスクから自動呼び出し（PR 作成前 / マージ前）
...
1. **`<button>` → `<Link>` 化**: `getByRole('button', ...)` が爆発する。利用者の権限や課金状態でシナリオを分けている spec ほど取りこぼしが起きやすい
...
6. **取りこぼしパターン**: 同じ画面を複数テストが叩いている場合、片方が `.first()` 等で修正済み、他方が未対応のまま残っているケースを検出する。具体的には: フロント変更で「同じ画面のセレクタ修正が必要」と判断したら、その画面を叩く **すべての** E2E テストに同じ修正が入っているかを横断確認する。横断確認の grep 起点戦略: (a) 画面の URL path（例: `/<セクション>/<リソース>/.+/edit` のような編集画面のパターン）、(b) 画面固有要素のテキスト（例: `保存`, `編集`）、(c) スクリーン名コメント、の3軸を併用する
...
- staging 環境を叩く spec（`@staging` のようなタグで区別されているもの）は **production 近接** で爆発時のコストが最大なので最優先
- テストファイルを丸読みする必要はない。**フロント変更が触る画面を起点に、関連テストだけ深読み** する
...
| 「E2E テストファイル名が違うから関係ない」 | NO。リソース単位で切った spec と利用者の権限単位で切った spec は、別ファイルでも同じ編集画面を叩くことがある。**画面単位** で関連テストを横断検出する |
| 「agent 任せだと誤検知が多い」 | confidence: low の指摘は人間判断で吸収する。本 skill の価値は「true positive を見逃さないこと」優先で、false positive は許容する |
| 「staging テストはステージング環境がないと検証できない」 | 本 skill は **静的検査のみ**。実行はしない。「壊れる可能性が高い」を伝えれば、利用者が修正判断を下せる |
| 「dev-workflow:pre-merge-quality-gate が動くから本 skill 単独呼び出しは不要」 | 大きいフロント変更（a11y 一括リファクタなど）の **完了直後** に単独で呼ぶと、PR を出す前に E2E 取りこぼしを検出できる。dev-workflow:pre-merge-quality-gate より早い shift-left ポイント |

## 関連

- `dev-workflow:pre-merge-quality-gate` (sibling skill): Phase 1 の4つ目並列タスクとして本 skill を呼ぶ
...
本 skill は、プロジェクトの CLAUDE.md が次のような規定を持つときに噛み合う。

- ローカル E2E を 1 ブラウザに絞り、クロスブラウザは CI のステージングシナリオテストで担保する
- マージ前にローカルマシン上で E2E を全パスさせることを求める
- プロダクションコードの変更時に、ユニット / 統合 / E2E だけでなく E2E シナリオテストへの影響調査まで求める

本 skill はこの「E2E シナリオテストへの影響調査」を **構造化された agent ワークフロー** で実行する。
規定が無いプロジェクトでも動くが、staging E2E が唯一の自動検出網になっている構成でいちばん効く。
