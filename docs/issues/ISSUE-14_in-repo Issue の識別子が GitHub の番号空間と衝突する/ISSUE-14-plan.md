...
- **既存の参照を書き換える規則** (Task 2 / 3 / 4 が従う)
  ...
  - **private リポジトリを名指ししない。**このリポジトリは PUBLIC で、CLAUDE.md が
    「private リポジトリの内部事情を書かない。一般化できる知見だけを出所を伴わない形で書く」を
    MUST にしている。リポジトリ名・org 名・向こうの Issue 番号・件数はすべて落とし、
    機序と一般化した知見だけを書く。**`gitleaks` の緑を根拠にしないこと** —
    検出集合の canonical は `.gitleaks.toml` で、リポジトリ名やプロジェクト名は
    そこに無いカテゴリなので素通りする (実測: 今回の露出は全 hook 緑のまま通った)
  - 他リポジトリの GitHub Issue / PR を指す必要が生じたら、それが PUBLIC であることを
    `gh repo view --json visibility` で確認してから `<owner>/<repo>#<N>` を使う。
    PRIVATE なら番号ごと落として散文で書く。
    なお相手が記法分離を採っていないリポジトリの in-repo Issue には `#` を使えない
    (`#` を付けるとその番号の GitHub オブジェクトへ解決する。実測で予約ダミーや
    無関係な Dependabot PR を指す例を確認済み)
