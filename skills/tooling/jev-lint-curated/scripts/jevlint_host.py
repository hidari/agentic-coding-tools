...
# 上流が worktree の中で自分で呼ぶ `git diff <base>...HEAD` は、partial clone の上で
# lazy fetch (利用者の remote.origin.uploadpack のコマンドを cwd=root で起動し、worktree に
# ファイルを書く) と `refs/replace` の差し替え追従の両方を実際に起こす (実測: git 2.55.0)。
# この 2 つを "1" にすると rc 128 で何も書かずに fail closed する (実測)。
# `git help git` (Homebrew の git 2.55.0) の該当箇所:
...
# jev-lint 固有の設定を渡さないための denylist。上流が読む固有の変数 (`JEV_LINT_MODEL` 等) は
# すべて `JEV_LINT_` で始まるのでこの 1 本の prefix で足りる。
_INSTALL_DROPPED_PREFIXES = ("TYPESAFE_", "TYPESAFEAI_", "JEV_LINT_")

# 取得の env から落とす変数名 (完全一致)。取得の段で置かれた host は、次のキー付きの起動が
# そのまま信じて走らせる (`build_env` は NODE_* を落とすが host の中身は見ない) ので、取得物を
# 差し替えられる変数を pnpm と `@ast-grep/cli` の postinstall に見せない。
# - `NODE_OPTIONS`: pnpm と postinstall の node に `--require` で任意のコードを読み込ませられる
# - `NODE_TLS_REJECT_UNAUTHORIZED`: "0" で TLS の検証を切り、registry との間の中間者が別物を置ける
# - `COREPACK_NPM_REGISTRY` / `COREPACK_INTEGRITY_KEYS`: PATH の pnpm が corepack の shim の
#   とき、pnpm 本体の取得先と、その署名を検証する鍵を差し替える (空にすると検証を切れる。
#   corepack の README の記載で、ここでは実測していない)
_INSTALL_DROPPED_NAMES = (
    "NODE_OPTIONS",
...
def _sanitize_path(value: str) -> str:
    ...
    実測: `PATH=node_modules/.bin:/usr/bin:/bin` は
    ...
    何も残らなければ `HostError` にする。`PATH=""` のまま渡すと `os.get_exec_path` は
    `['']` を返し (実測: 3.9.6 と 3.14.7)、ラッパ自身の最初の git がプロセスの cwd の
    `./git` として起動される。`PATH` が無いとき (呼び出し側はこの関数を呼ばず、子プロセスは
    `os.defpath` で探す) とは区別する。
    """
    parts = [part for part in value.split(":") if part and part.startswith("/")]
    if not parts:
        raise HostError(f"PATH に絶対パスの要素が無い (相対と空の要素は使わない): {value!r}")
    return ":".join(parts)
...
def _is_npm_or_pnpm_config(name: str) -> bool:
    ...
    利用者や CI 設定が書く大文字形。実測: pnpm 12.3.4 は `pnpm add`
...
def install_env(source: Mapping[str, str]) -> dict:
    ...
    `TYPESAFEAI_API_KEY`)、上流固有の変数 (`TYPESAFE_BASE_URL` 等)、取得物を差し替えられる
    node と corepack の変数 (`_INSTALL_DROPPED_NAMES`)、そして `npm_config_*` /
    `PNPM_CONFIG_*` を落とす。最後のものを落とす理由: pnpm は
    ...
    proxy の変数 (`HTTPS_PROXY` 等) と `NODE_EXTRA_CA_CERTS` は落とさない。社内の proxy や
    TLS を終端する中継を通る構成では、これが無いと取得そのものができない。
    `NODE_EXTRA_CA_CERTS` は信頼する CA を足すだけで、既定の検証は切らない。
    ...
    env = {
        ...
        and name not in _INSTALL_DROPPED_NAMES
        and not _is_npm_or_pnpm_config(name)
    }
    if "PATH" in env:
...
# host の中のレイアウト (`pnpm add` が置く形) はこの 3 つの関数だけが持つ。再利用の判定、
# engines の読み取り、起動の argv が別々に書くと、上流が置き場を変えたときに片方だけ直る
def _package_dir(host: Path) -> Path:
    return host / "node_modules" / "jev-lint"


def package_json_path(host: Path) -> Path:
    """host に取得した jev-lint の `package.json`。"""
    return _package_dir(host) / "package.json"


def cli_path(host: Path) -> Path:
    """host に取得した jev-lint の起動の入口 (node で直接呼ぶ)。"""
...
def default_run(argv: list, cwd: str, env: Mapping[str, str]) -> "subprocess.CompletedProcess":
    """`prepare_host` の `run` の既定実装。pnpm の stdout をラッパ自身の stderr へ流す。

    ラッパの stdout は check / review の要約 (`jevlint_result.summarize` が返すプレーン
    テキスト) と compat の報告だけにするため、pnpm 自身の進捗表示 (`Progress: resolved ...`
    等) を混ぜない。`subprocess.run` をそのまま使うと、初回の取得で進捗が stdout に混ざる。
    stderr は継承したまま (pnpm 自身のエラーメッセージは利用者にそのまま見えたほうが診断
    しやすい)。
    """
...
def prepare_host(
    version: str, source: Mapping[str, str], run: Callable = default_run
) -> Path:
    """host を再利用できればそれを返し、できなければ取得してから host の名前に置く。

    pnpm を worktree や消費側のリポジトリを cwd にして起動しない理由 (実測: node 24.18.0 /
    pnpm 12.3.4 / git 2.55.0): pnpm は cwd の `.npmrc` の `registry=` を読み、`dlx` の
    キャッシュが温まっていても読む (キャッシュのキーが registry を含むため)。環境変数
    `npm_config_registry` はこの project の `.npmrc` に負ける。消費側がコミットした
    `.npmrc` のある worktree で `pnpm dlx` を起動すると、偽の registry が返す別物の
    jev-lint がキー付きの env で走ることをダミーのキーで再現した。この関数は host の親の
    中に作った一時ディレクトリ (利用者の cache ディレクトリの下で、どの消費側のリポジトリ
    にも属さない) を cwd にして `pnpm add` を 1 回だけ呼ぶ。
...
def fixed_tail(config: Path) -> list:
    ...
    フラグは後に書いたものが勝つので、消費側の値 (`--base` / `--exclude` /
