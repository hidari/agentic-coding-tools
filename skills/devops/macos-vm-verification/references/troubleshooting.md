...
`error: 転送後のサイズが一致しません` は「照合が外れた」であって「転送が切れた」の断定では
ない。この順で切り分ける。

1. VM 側の実体を見る (`macvm exec --host <alias> -- 'ls -lL <remote>'`)。正しいサイズで在るなら
   転送は成功していて、外れたのは照合の側
2. ControlMaster の接続が古くなっていることがある。`~/.ssh/cm-*` を消してから再試行する

`error: 転送後のサイズ照合ができませんでした (ssh の失敗: ...)` は照合の ssh 自体が落ちた
ケースで、ファイルは届いている可能性がある。転送をやり直す前に `macvm doctor` で接続を見る。

なお転送先をディレクトリで渡す書き方 (`w/` や空文字) は exit 2 で拒む。scp はディレクトリ宛だと
その中へ置くのにサイズ照合は渡された文字列をそのまま見るので、通すと成功した転送を
「途中で切れた」と誤報する。ゲスト側に同名のディレクトリが既に在る場合は書き方から判別
できないので通る (その場合は上の 1 で実体を見る)。
