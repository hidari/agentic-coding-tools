...
ことが強制される。main を保護して同梱経路を既定にしているこのリポジトリではそれが正しいが、
post-merge クローズを既定とするリポジトリとは両立しない。**そのまま配布しないこと。**

「直 push を選べない」とは書かない。ruleset の bypass_actors にオーナーが入っているので
直 push は実行でき、しかも rule を返す endpoint は bypass 特権を反映しないため判定手順から
その差は見えない (ISSUE-41 の spec が 2026-08-29 に記録し、ISSUE-48 が再実測した)。
上の宣言が乗っているのは不可能性ではなく方針である。
